...
class Powerplant(object):
    def __init__(self, dataset_dir=None, subsample_ratio=1.0, test_ratio=0.1, split_seed=0, **kwargs):
        if dataset_dir is None:
            self.dataset_dir = '/datasets/uci/powerplant'
        else:
            self.dataset_dir = dataset_dir
        self.train_dataset, self.test_dataset = self._preprocess(subsample_ratio, test_ratio,
                                                                 split_seed)

    def _preprocess(self, subsample_ratio, test_ratio, split_seed):
        file_path = os.path.join(self.dataset_dir, 'Folds5x2_pp.xlsx')
        raw_data = np.array(pd.read_excel(file_path))
        inputs = torch.tensor(raw_data[:, :-1], dtype=torch.get_default_dtype())
        targets = torch.tensor(raw_data[:, -1], dtype=torch.get_default_dtype()).unsqueeze(-1)
